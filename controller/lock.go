...
func ClearStatus(c *gin.Context) {
	...
	// อัปเดตข้อมูล
	result := db.Model(&entity.Locks{}).Where("status = ?", updateRequest.StatusToClear).Update("status", updateRequest.NewStatus)
	if result.Error != nil {
...
func CountReservations(c *gin.Context) {
	...
	db := config.DB()
	result := db.Table("reserves").Select("SUM(total_price)").Scan(&totalReservationsPrice)
